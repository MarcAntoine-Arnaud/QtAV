...
MainWindow::MainWindow( QWidget *parent )
	...
	, mpRenderer     ( 0 )
	, mpTempRenderer ( 0 )
	, mShowControl   ( eShowControlAlways )
	, mTimerId       ( 0 )
	, mIsReady       ( false )
	, mHasPendingPlay( false )
	, mNullAO        ( false )
{
	setWindowTitle( "avCesar" );
...
void MainWindow::setupUi()
{
	...
	int width    = mPlayPixmap.width();
	int height   = mPlayPixmap.height();
	mPausePixmap = mPlayPixmap.copy( QRect( 0.5 * width, 0, 0.5 * width, height ));
	mPlayPixmap  = mPlayPixmap.copy( QRect( 0, 0, 0.5 * width, height ));

	qDebug("pixmap %d x %d", mPlayPixmap.width(), mPlayPixmap.height());

	mpPlayPauseBtn = new Button(mpControl);
	int a = qMin( width, height );
	const int kMaxButtonIconWidth = 30;
	...
	mpVolumeSlider->setMaximumHeight(20);
...
void MainWindow::setAudioOutput(AudioOutput* )
{
...
void MainWindow::onStartPlay()
{
	...
	setVolume();
	mTimerId = startTimer( kSliderUpdateInterval );
	mShowControl = eShowControlCanHide;
...
{
	AudioOutput *audioPlayer = mpPlayer ? mpPlayer->audio() : 0;
	qreal volume = qreal( mpVolumeSlider->value() ) * kVolumeInterval;
	...
	mpVolumeSlider->setToolTip( QString::number( volume ) );
	mpVolumeBtn->setToolTip( QString::number( volume ) );
}
...
void MainWindow::mouseMoveEvent( QMouseEvent* e )
{
	...
	{
		if( mShowControl == eShowControlCanHide )
		{
			mShowControl = eShowControlPlaying;
			tryShowControlBar();
		}
	}
	else
	{
		if( mShowControl == eShowControlPlaying )
		{
			mShowControl = eShowControlCanHide;
			QTimer::singleShot( 3000, this, SLOT( tryHideControlBar() ) );
		}
...
{
	qreal ratio = action->data().toDouble();
	if( action == mpARAction && ratio != -2 )
	{
		action->toggle(); //check state changes if clicked
		return;
	}
	...
	else
	{
		...
		{
			if( ratio == -2 )
				ratio = QInputDialog::getDouble(0, tr( "Aspect ratio" ), "", 1.0 );

			mpPlayer->renderer()->setOutAspectRatioMode( VideoRenderer::CustomAspectRation );
			mpPlayer->renderer()->setOutAspectRatio( ratio );
		}
	}
