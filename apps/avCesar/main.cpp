...
int main( int argc, char *argv[] )
{
	...
	{
		qDebug("Usage: %s [-vo qt/gl/d2d/gdi] [url/path]filename", app.applicationFilePath().section(QDir::separator(), -1).toUtf8().constData());
		qDebug("\n%s", aboutQtAV_PlainText().toUtf8().constData());
		return 0;
	}

	QStringList qms;
	qms << "avCesar" << "player" << "qt";
	...
	QString vo;
	int idx = app.arguments().indexOf( "-vo" );
	if( idx > 0 )
	{
		vo = app.arguments().at(idx+1);
	}
	else
	{
	...
	QString title = "avCesar " /*+ vo + " "*/ + QtAV_Version_String_Long();
	...
	if( ! renderer)
	{
		QMessageBox::critical(0, "avCesar", "vo '" + vo + "' not supported");
		return 1;
	}
	...
	idx = app.arguments().indexOf("-ao");
	if (idx > 0)
	{
		ao = app.arguments().at(idx+1);
	}
	ao = ao.toLower();
